...
class TestTor(unittest.TestCase):
    ...
    def test_post_request(self):
        response = self.tor.post_request('http://httpbin.org/post', request_body={'key': 'value'})
        self.assertEqual(response.status_code, 200)

    def test_get_ip(self):
        ip = self.tor.get_ip(tor_ip=True)
        self.assertRegex(ip, r'\b(?:\d{1,3}\.){3}\d{1,3}\b')

    def test_renew_tor_ip(self):
        old_ip = self.tor.get_ip(tor_ip=True)
        self.tor.get_new_ip(max_retries=5)
        new_ip = self.tor.get_ip(tor_ip=True)
